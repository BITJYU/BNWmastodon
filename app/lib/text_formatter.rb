...
class TextFormatter
  ...
  def rewrite
    ...
    last_index = entities.reduce(0) do |index, entity|
      ...
      result << format_markdown_fragment(text[index...indices.first])
      result << yield(entity)
      indices.last
    end

    result << format_markdown_fragment(text[last_index..])

    result
  end

  def format_markdown_fragment(fragment)
    escaped = h(fragment)
    escaped.gsub!(BOLD_REGEX, '<strong>\1</strong>')
    escaped.gsub!(ITALIC_REGEX, '<em>\1</em>')
    escaped
  end
