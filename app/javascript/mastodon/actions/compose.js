...
const toScheduledDate = scheduledAt => {
  if (!scheduledAt) {
    return null;
  }
...
export function submitCompose(routerHistory) {
  return function (dispatch, getState) {
    ...
    const scheduledAt = statusId === null ? getState().getIn(['compose', 'scheduled_at'], '') : '';
    ...
    api(getState).request({
      ...
      data: {
        ...
        scheduled_at: toScheduledDate(scheduledAt),
      },
      headers: {
        'Idempotency-Key': getState().getIn(['compose', 'idempotencyKey']),
      },
    }).then(function (response) {
      const isScheduledStatus = Object.prototype.hasOwnProperty.call(response.data, 'scheduled_at') && Object.prototype.hasOwnProperty.call(response.data, 'params');

      if (!isScheduledStatus && routerHistory && (routerHistory.location.pathname === '/publish' || routerHistory.location.pathname === '/statuses/new') && window.history.state) {
...
export function changeComposeSchedule(scheduledAt) {
  return {
