 ...
 * @param {number} [options.fallbackDelay]
 * @param {number} [options.fallbackInterval]
  ...
  return connectStream(channelName, params, (dispatch, getState) => {
    ...
    const useFallback = fallback => {
      fallback(dispatch, () => {
        ...
        pollingId = setTimeout(() => useFallback(fallback), options.fallbackInterval ?? (20000 + randomUpTo(20000)));
      });
    };
    ...
    return {
      ...
      onDisconnect() {
        ...
        if (options.fallback) {
          // @ts-expect-error
          pollingId = setTimeout(() => useFallback(options.fallback), options.fallbackDelay ?? randomUpTo(40000));
        }
      },
...
const refreshCommunityTimeline = ({ onlyMedia } = {}) => (dispatch, done) =>
  // @ts-expect-error
  dispatch(expandCommunityTimeline({ onlyMedia }, done));
...
export const connectCommunityStream = ({ onlyMedia } = {}) =>
  connectTimelineStream(`community${onlyMedia ? ':media' : ''}`, `public:local${onlyMedia ? ':media' : ''}`, {}, {
    ...
    fallback: refreshCommunityTimeline({ onlyMedia }),
    fallbackDelay: 2000,
    fallbackInterval: 5000,
  });
