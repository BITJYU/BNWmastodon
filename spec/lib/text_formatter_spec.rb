...
RSpec.describe TextFormatter do
  describe '#to_s' do
    ...
    context 'when given text containing bold markdown' do
      let(:text) { '**bold**' }

      it 'renders the text as bold' do
        expect(subject).to eq '<p><strong>bold</strong></p>'
      end
    end

    context 'when given text containing italic markdown' do
      let(:text) { '*italic*' }

      it 'renders the text as italic' do
        expect(subject).to eq '<p><em>italic</em></p>'
      end
    end
